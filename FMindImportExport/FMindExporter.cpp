...
bool CFMindExporter::Export(const ITaskList* pSrcTaskFile, LPCTSTR szDestFilePath, BOOL /*bSilent*/, IPreferences* /*pPrefs*/, LPCTSTR /*szKey*/)
{
	...
	ExportTask(pITL10, pITL10->GetFirstTask(), firstItem , 0);

	// save output manually to restore non-escaping of & and <>
	CString sOutput = Export(fileDest);
	return (FileMisc::SaveFile(szDestFilePath, sOutput, SFE_MULTIBYTE) == TRUE);
}
...
bool CFMindExporter::Export(const IMultiTaskList* pSrcTaskFile, LPCTSTR szDestFilePath, BOOL /*bSilent*/, IPreferences* /*pPrefs*/, LPCTSTR /*szKey*/)
{
	...
	for (int nTaskList = 0; nTaskList < pSrcTaskFile->GetTaskListCount(); nTaskList++)
	{
		...
		if (pITL8)
		{
			...
			// export first task
			ExportTask(pITL8, pITL8->GetFirstTask(), firstItem , 0);
		}
	}
...
void CFMindExporter::ExportTask(const ITaskList10* pSrcTaskFile, HTASKITEM hTask, CXmlItem* pXIDestParent, int LEVEL)
{
	if (!hTask)
		return;
	...
	if (!sComments.IsEmpty())
	{
		...
		CStringArray aParas;
		int nNumPara = Misc::Split(sComments, '\n', aParas);
