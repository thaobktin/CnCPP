...
CString CToDoCtrlFind::GetLongestAllocTo(const CString& sExtra, BOOL bVisibleOnly) const
{
	return GetLongerString(sExtra, GetLongestAllocTo(NULL, NULL, bVisibleOnly));
}

CString CToDoCtrlFind::GetLongestCategory(const CString& sExtra, BOOL bVisibleOnly) const
{
	return GetLongerString(sExtra, GetLongestCategory(NULL, NULL, bVisibleOnly));
}

CString CToDoCtrlFind::GetLongestTag(const CString& sExtra, BOOL bVisibleOnly) const
{
	return GetLongerString(sExtra, GetLongestTag(NULL, NULL, bVisibleOnly));
}
...
CString CToDoCtrlFind::GetLongestCost(BOOL bVisibleOnly) const
{
	return GetLongestCost(NULL, NULL, NULL, bVisibleOnly);
}

CString CToDoCtrlFind::GetLongerString(const CString& str1, const CString& str2) const
{
...
CString CToDoCtrlFind::GetLongestCost(HTREEITEM hti, const TODOITEM* pTDI, const TODOSTRUCTURE* pTDS, BOOL bVisibleOnly) const
{
	...
	if (WantSearchChildren(hti, bVisibleOnly))
	{
		...
		while (htiChild)
		{
			CString sChildLongest = GetLongestCost(htiChild, NULL, NULL, bVisibleOnly);
			sLongest = Misc::GetLongest(sLongest, sChildLongest);
