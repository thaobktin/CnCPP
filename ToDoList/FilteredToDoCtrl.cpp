...
BOOL CFilteredToDoCtrl::LoadTasks(const CTaskFile& file)
{
	...
	if (bViewWasUnset)
	{
		...
		case FTCV_UIEXTENSION16:
			...
			// extension so we only need to update is the tree actually
			// has a filter
			if (IsFilterSet(FTCV_TASKTREE))
...
void CFilteredToDoCtrl::BuildFilterQuery(SEARCHPARAMS& params) const
{
	...
	case FS_RECENTMOD:
		params.aRules.Add(SEARCHPARAM(TDCA_RECENTMODIFIED, FO_SET));
		break;
	...
	if (InitFilterDate(m_filter.nStartBy, dateStart))
	{
		...
		if (m_filter.nStartBy == FD_NOW)
			params.aRules.Add(SEARCHPARAM(TDCA_STARTTIME, FO_ON_OR_BEFORE, dateStart));
		else
			params.aRules.Add(SEARCHPARAM(TDCA_STARTDATE, FO_ON_OR_BEFORE, dateStart));
	...
	if (InitFilterDate(m_filter.nDueBy, dateDue))
	{
		...
		if (m_filter.nDueBy == FD_NOW)
			params.aRules.Add(SEARCHPARAM(TDCA_DUETIME, FO_ON_OR_BEFORE, dateDue));
		else
			params.aRules.Add(SEARCHPARAM(TDCA_DUEDATE, FO_ON_OR_BEFORE, dateDue));
...
void CFilteredToDoCtrl::OnTimer(UINT nIDEvent) 
{
	...
	case TIMER_NOWFILTER:
		...
		// 3. We're already displaying all tasks
		if (m_tree.GetCount() == m_data.GetTaskCount())
			return;

		// iterate the full data looking for items not in the
		// tree and test them for inclusion in the filter
		CHTIMap htiMap;
		VERIFY (m_tree.BuildHTIMap(htiMap) < (int)m_data.GetTaskCount());
