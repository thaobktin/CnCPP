...
int CTaskFile::GetCustomAttributeDefs(const ITaskList* pTasks, CTDCCustomAttribDefinitionArray& aAttribs)
{
	...
	if (pTasks10)
	{
		...
		for (int nCustom = 0; nCustom < nNumCustom; nCustom++)
		{
			...
			attribDef.dwFeatures = _ttoi(pTasks10->GetCustomAttributeValue(nCustom, TDL_CUSTOMATTRIBSORTABLE));
...
int CTaskFile::GetCustomAttributeDefs(CTDCCustomAttribDefinitionArray& aAttribs) const
{
	...
	while (pXIAttribDef)
	{
		...
		if (attribDef.dwFeatures == 0)
			attribDef.dwFeatures = pXIAttribDef->GetItemValueI(TDL_CUSTOMATTRIBSORTABLE);
...
{
	return GraphicsMisc::GetWebColor(GetTaskPriorityColor(hTask));
}
...
{
	return (CDateHelper::IsDateSet(GetTaskDoneDateOle(hTask)) ||
...
BOOL CTaskFile::SetTaskTextColors(HTASKITEM hTask, COLORREF crText, COLORREF crBack)
{
	if (!SetTaskULong(hTask, TDL_TASKTEXTCOLOR, crText) ||
		!SetTaskString(hTask, TDL_TASKTEXTWEBCOLOR, GraphicsMisc::GetWebColor(crText)))
		return FALSE;

	if (crBack != CLR_NONE &&
		(!SetTaskULong(hTask, TDL_TASKBACKCOLOR, crBack) ||
		!SetTaskString(hTask, TDL_TASKBACKWEBCOLOR, GraphicsMisc::GetWebColor(crBack))))
		return FALSE;
