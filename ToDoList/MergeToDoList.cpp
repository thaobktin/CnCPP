...
void CMergeToDoList::BuildDestLookupMap()
{
	...
	if (m_pXIDestRoot->GetItemCount())
	{
		...
		if (m_dwNextID)
		{
			...
			AddDestTaskToLookupMap(m_pXIDestRoot);
		}
	}
}

void CMergeToDoList::AddDestTaskToLookupMap(CXmlItem* pXIDest)
{
	if (!pXIDest)
		return;
	...
	// first child
	AddDestTaskToLookupMap(pXIDest->GetItem(TDL_TASK));

	// next sibling
	AddDestTaskToLookupMap(pXIDest->GetSibling());
}
