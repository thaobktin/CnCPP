...
struct TDCFINDWND
{
	TDCFINDWND(HWND ignore = NULL) : hWndIgnore(ignore) 
	{
	}
...
struct TDCSTARTUP
{
	...
	TDCSTARTUP(const CEnCommandLineInfo& cmdInfo)
	{
		...
		if (cmdInfo.HasOption(SWITCH_STAGING))
			dwFlags |= TLD_STAGING;
...
struct TDCCUSTOMATTRIBUTEDEFINITION
{
	...
	TDC_COLUMN GetColumnID() const { return nColID; }
	TDC_ATTRIBUTE GetAttributeID() const { return nAttribID; }
	DWORD GetAttributeType() const { return dwAttribType; }
	...
	BOOL HasFeature(DWORD dwFeature) const { return (dwFeatures & dwFeature); }
