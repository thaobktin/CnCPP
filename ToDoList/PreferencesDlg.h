...
public:
	...
	BOOL GetEnableAdd2Dictionary() const { return m_pageGen.GetEnableAdd2Dictionary(); }
	...
	UINT GetAutoCheckinFrequency() const { return m_pageMultiUser.GetAutoCheckinFrequency(); }
	BOOL GetUsing3rdPartySourceControl() const { return m_pageMultiUser.GetUsing3rdPartySourceControl(); }
	UINT GetIncludeUserNameInCheckout() const { return m_pageMultiUser.GetIncludeUserNameInCheckout(); }
	...
	int GetDefaultTags(CStringArray& aTags) const { return m_pageTaskDef.GetDefaultTags(aTags); }
	CString GetDefaultCreatedBy() const { return m_pageTaskDef.GetDefaultCreatedBy(); }
	double GetDefaultTimeEst(TCHAR& nUnits) const { return m_pageTaskDef.GetDefaultTimeEst(nUnits); }
	double GetDefaultTimeSpent(TCHAR& nUnits) const { return m_pageTaskDef.GetDefaultTimeSpent(nUnits); }
	...
	int GetDefaultListItems(PTDP_LIST nList, CStringArray& aItems) const { return m_pageTaskDef.GetListItems(nList, aItems); }
	double GetDefaultCost() const { return m_pageTaskDef.GetDefaultCost(); }
	BOOL GetCategoryListIsReadonly() const { return m_pageTaskDef.GetCategoryListIsReadonly(); }
	BOOL GetStatusListIsReadonly() const { return m_pageTaskDef.GetStatusListIsReadonly(); }
	BOOL GetAllocToListIsReadonly() const { return m_pageTaskDef.GetAllocToListIsReadonly(); }
	BOOL GetAllocByListIsReadonly() const { return m_pageTaskDef.GetAllocByListIsReadonly(); }
	...
	int GetDefaultColumns(CTDCColumnIDArray& aColumns) const { return m_pageUITasklist.GetDefaultColumns(aColumns); }
	void SetDefaultColumns(const CTDCColumnIDArray& aColumns) { m_pageUITasklist.SetDefaultColumns(aColumns); }
