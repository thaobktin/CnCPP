...
BOOL CPreferencesToolPage::OnInitDialog() 
{
	...
	for (int nTool = 0; nTool < m_aTools.GetSize(); nTool++)
	{
      CString sToolPath = m_aTools[nTool].sToolPath;
		...
		nImage = m_ilSys.GetFileImageIndex(sToolPath);
...
LRESULT CPreferencesToolPage::OnEEBtnClick(WPARAM wParam, LPARAM lParam)
{
	...
	case IDC_CMDLINE:
		if (lParam == 1)
		{
			...
			if (menu.LoadMenu(IDR_PLACEHOLDERS, NULL, TRUE))
			{
...
void CPreferencesToolPage::OnSize(UINT nType, int cx, int cy) 
{
	...
	if (m_lcTools.GetSafeHwnd())
	{
		...
		CDialogHelper::GetChildControlIDs(this, aCtrls);
		CDialogHelper::RemoveControlID(IDC_TOOLLIST, aCtrls);
		CDialogHelper::RemoveControlID(IDC_DELETETOOL, aCtrls);
		CDialogHelper::RemoveControlID(IDC_IMPORT, aCtrls);
		...
		CDialogHelper::ResizeCtrl(this, IDC_TOOLLIST, nXOffset, nYOffset);
	}
}
