...
const UINT RECALCALLCOLS = 0xffff;
						...
						m_nMaxState(TDCMS_NORMAL)
{
	...
	CDateHelper::ClearDate(s_tdDefault.dateCreated);
...
short CToDoCtrl::CalcMinItemImageHeight() const
{
	...
	if (m_tree.GetGridlineColor() == CLR_NONE)
		return 16;
...
void CToDoCtrl::UpdateControls(HTREEITEM hti)
{
	...
	if (hti)
	{
		...
		SetCtrlDate(m_dateDue, dateDue, dateStart);
	...
	case TDCA_DONEDATE:
		{
			...
			m_timeDone.EnableWindow(CDateHelper::IsDateSet(date));
		}
		break;
	...
	case TDCA_STARTDATE:
		{
			...
			m_timeStart.EnableWindow(CDateHelper::IsDateSet(date));
		}
		break;
	...
	case TDCA_DUEDATE:
		{
			...
			m_timeDue.EnableWindow(CDateHelper::IsDateSet(date));
		}
		break;
...
void CToDoCtrl::SetDefaultComboNames(CAutoComboBox& combo, const CStringArray& aNewNames, 
									 CStringArray& aDefNames, BOOL bReadOnly)
{
	CStringArray aComboItems;
	CDialogHelper::GetComboBoxItems(combo, aComboItems);

	// remove the old names
	Misc::RemoveItems(aDefNames, aComboItems);

	// add new names
	Misc::AddUniqueItems(aNewNames, aComboItems);
	aDefNames.Copy(aNewNames);

	// recreate combo as required
	VERIFY(CDialogHelper::SetComboReadOnly(combo, bReadOnly));

	// re-add items
	CDialogHelper::SetComboBoxItems(combo, aComboItems);
}

void CToDoCtrl::SetDefaultAllocToNames(const CStringArray& aNames, BOOL bReadOnly)
{
	SetDefaultComboNames(m_cbAllocTo, aNames, m_aDefAllocTo, bReadOnly);

	OnSelCancelAllocTo();
}

void CToDoCtrl::SetDefaultAllocByNames(const CStringArray& aNames, BOOL bReadOnly)
{
	SetDefaultComboNames(m_cbAllocBy, aNames, m_aDefAllocBy, bReadOnly);

	// restore selection
	m_cbAllocBy.SelectString(0, m_sAllocBy);
}

void CToDoCtrl::SetDefaultCategoryNames(const CStringArray& aNames, BOOL bReadOnly)
{
	SetDefaultComboNames(m_cbCategory, aNames, m_aDefCategory, bReadOnly);
	
	OnSelCancelCategory();
}

void CToDoCtrl::SetDefaultTagNames(const CStringArray& aNames, BOOL bReadOnly)
{
	SetDefaultComboNames(m_cbTags, aNames, m_aDefTags, bReadOnly);
	
	OnSelCancelTag();
}

void CToDoCtrl::SetDefaultStatusNames(const CStringArray& aNames, BOOL bReadOnly)
{
	SetDefaultComboNames(m_cbStatus, aNames, m_aDefStatus, bReadOnly);
	...
	if (nRes == SET_CHANGE)
	{
		...
		case TDCD_STARTDATE:
			...
			if (bDateEdited && CDateHelper::IsDateSet(date))
			{
				...
				{
					SetCtrlDate(m_dateDue, 0.0, date);
				}
			}
			// fall thru
		...
		case TDCD_DUEDATE:	
			...
			if (bDateEdited && !CDateHelper::IsDateSet(date))
			{
				...
				SetCtrlDate(m_dateDue, 0.0, dtStart);
			}
			// fall thru
...
BOOL CToDoCtrl::SelectionHasReferences() const
{
	POSITION pos = Selection().GetFirstItemPos();
...
BOOL CToDoCtrl::SelectionHasNonReferences() const
{
	POSITION pos = Selection().GetFirstItemPos();
											 ...
											 const COleDateTime& dtNext, BOOL bDueDate)
{
	...
	if (bDueDate) // dtNext is the new due date
	{
		...
		else // bump dates by required amount
		{
			if (CDateHelper::IsDateSet(dtDue))
			{
				int nOffsetDays = (int)(dtNext - dtDue);
				
				m_data.OffsetTaskDate(dwNewTaskID, TDCD_DUEDATE, nOffsetDays, DHU_DAYS, TRUE, FALSE);
				m_data.OffsetTaskDate(dwNewTaskID, TDCD_STARTDATE, nOffsetDays, DHU_DAYS, TRUE, FALSE);
			}
			else
				m_data.SetTaskDate(dwNewTaskID, TDCD_DUE, dtNext);
		}
	...
	{
		if (WantUpdateInheritedAttibute(TDCA_STARTDATE))
		{
			...
			if (CDateHelper::IsDateSet(dtStart) && CDateHelper::IsDateSet(dtDue))
			{
				...
				ApplyLastInheritedChangeToSubtasks(dwNewTaskID, TDCA_DUEDATE);
			}
		}
		else // bump dates by required amount
		{
			...
			{
				int nOffsetDays = (int)(dtNext - dtStart);
...
LRESULT CToDoCtrl::OnRecreateRecurringTask(WPARAM wParam, LPARAM /*lParam*/)
{
	...
	m_data.FixupTaskDependents(dwNewTaskID, dwTaskID);
...
int CToDoCtrl::SetTaskDone(HTREEITEM hti, const COleDateTime& date, TDC_SETTASKDONE nSetChildren)
{
	...
	if (m_data.SetTaskDate(dwTaskID, TDCD_DONE, date) == SET_CHANGE)
	{
		...
		if (!m_sCompletionStatus.IsEmpty())
		{
			...
			else if (!bDone && bWasDone && (m_data.GetTaskStatus(dwTaskID) == m_sCompletionStatus))
			{
...
BOOL CToDoCtrl::SetSelectedTaskTimeEstimate(double dTime, TCHAR nUnits)
{
	...
	while (pos)
	{
		...
		if (!m_tree.ItemHasChildren(hti) || HasStyle(TDCS_ALLOWPARENTTIMETRACKING))
		{
			...
			if (nItemRes == SET_CHANGE)
			{
				ApplyLastInheritedChangeToSubtasks(dwTaskID, TDCA_TIMEEST);
				nRes = SET_CHANGE;
				dwModTaskID = dwTaskID;
			}
		}
...
TODOITEM* CToDoCtrl::CreateNewTask(HTREEITEM htiParent)
{
	...
	return m_data.NewTask(&s_tdDefault, dwParentID, &s_aParentAttribs);
}
...
BOOL CToDoCtrl::CanSplitSelectedTask()
{
	...
	if (nSelCount == 1)
	{
		if (IsSelectedTaskDone() || ItemHasChildren(GetSelectedItem()))
			return FALSE;
	}
...
BOOL CToDoCtrl::SplitSelectedTask(int nNumSubtasks)
{
	...
	if (!GetSelectedCount() || SelectionHasReferences())
		return FALSE;
...
BOOL CToDoCtrl::GetLabelEditRect(CRect& rScreen)
{
	...
	return GetItemTitleRect(htiSel, TDCTR_EDIT, rScreen);
}
...
void CToDoCtrl::UpdateColumnHeaderSorting()
{
	BOOL bEnable = HasStyle(TDCS_COLUMNHEADERSORTING);
	
	// standard gutter columns
	for (int nCol = 0; nCol < NUM_COLUMNS; nCol++)
	{
		TDCCOLUMN& tdcc = COLUMNS[nCol];

		m_tree.EnableGutterColumnHeaderClicking(tdcc.nColID, bEnable);
	}
	...
	for (int nAttrib = 0; nAttrib < m_aCustomAttribDefs.GetSize(); nAttrib++)
	{
		...
		if (bEnable)
			bEnable = CTDCCustomAttributeHelper::IsColumnSortable(nColID, m_aCustomAttribDefs);

		m_tree.EnableGutterColumnHeaderClicking(nColID, bEnable);
	}
}
...
void CToDoCtrl::RebuildCustomAttributeUI()
{
	...
	CTDCCustomAttributeHelper::RebuildCustomAttributeUI(m_aCustomAttribDefs, m_aCustomControls, 
														m_ilTaskIcons, m_tree, this, nPos, IDC_VERSION);
...
BOOL CToDoCtrl::LoadTasks(const CTaskFile& file)
{
	...
	m_tLastTaskMod = file.GetLastModifiedOle();
	...
	if (!pTDI->sComments.IsEmpty())
	{
		...
		if (!bFullRowSelected)
		{
			...
			else
			{
				...
				if (rgbBkGnd.Luminance() < 128)
					crComments = ALTCOMMENTSCOLOR;
			}
								 ...
								 COLORREF& crText, COLORREF& crBack) const
{
	...
	else // all incomplete tasks
	{
		...
		if (pTDI->dwTaskRefID && (m_crReference != CLR_NONE))
		{
		...
		else
		{
			...
			if (bDueToday && m_crDueToday != CLR_NONE)
			{
				crText = m_crDueToday;
			}
			// likewise for overdue
			else if (bOverDue && m_crDue != CLR_NONE)
			{
				crText = m_crDue;
			}
			// do we have a custom 'started today' color
			else if (bStartedToday && m_crStartedToday != CLR_NONE)
			{
				crText = m_crStartedToday;
			}
			// likewise for started
			else if (bStartedBefore && m_crStarted != CLR_NONE)
			{
				crText = m_crStarted;
			}
			else if (m_crFlagged != CLR_NONE && pTDI->bFlagged)
			{
				crText = m_crFlagged;
			}
...
BOOL CToDoCtrl::AccumulateRecalcColumn(TDC_COLUMN nColID, UINT& nRecalcColID) const
{
	// optimisation
	if (nRecalcColID == RECALCALLCOLS)
	{
		return TRUE;
	}
...
void CToDoCtrl::SetModified(BOOL bMod, TDC_ATTRIBUTE nAttrib, DWORD /*dwModTaskID*/)
{
	...
	if (bMod)
	{
		...
		case TDCA_STARTDATE:
			AccumulateRecalcColumn(TDCC_STARTDATE, nRecalcColID);
			break;
			
		case TDCA_EXTERNALID:
			AccumulateRecalcColumn(TDCC_EXTERNALID, nRecalcColID);
			break;
		...
		case TDCA_RECURRENCE:
			AccumulateRecalcColumn(TDCC_RECURRENCE, nRecalcColID);
			break;
		...
		case TDCA_ALLOCTO:
			AccumulateRecalcColumn(TDCC_ALLOCTO, nRecalcColID);
			break;
			
		case TDCA_ALLOCBY:
			AccumulateRecalcColumn(TDCC_ALLOCBY, nRecalcColID);
			break;
		...
		case TDCA_VERSION:
			AccumulateRecalcColumn(TDCC_VERSION, nRecalcColID);
			break;
			
		case TDCA_CATEGORY:
			AccumulateRecalcColumn(TDCC_CATEGORY, nRecalcColID);
			break;
			
		case TDCA_TAGS:
			AccumulateRecalcColumn(TDCC_TAGS, nRecalcColID);
		...
		case TDCA_COST:
			AccumulateRecalcColumn(TDCC_COST, nRecalcColID);
			break;
...
LRESULT CToDoCtrl::OnTreeDragOver(WPARAM /*wParam*/, LPARAM lParam)
{
	...
	if (nRes != DD_DROPEFFECT_NONE)
	{
		...
		if (pDDI->bLeftDrag)
		{
			HTREEITEM htiOver = m_tree.HitTest(pDDI->pt);
			DWORD dwOverTaskID = GetTaskID(htiOver);

			if (m_data.IsTaskReference(dwOverTaskID) &&
				SelectionHasNonReferences())
			{
	...
	if (nRes)
	{
		...
		if (pMsg->message == WM_DD_DRAGDROP)
		{
			...
			if (pDDI->bLeftDrag) 
			{
				...
				// as references
				if (bTargetIsRef && SelectionHasNonReferences())
				{
					bRef = TRUE;
				}
				else
			...
			else // right drag
			{
				...
				if (menu.LoadMenu(IDR_TREEDRAGDROP, *this, TRUE))
				{
					...
					else if (bTargetIsRef) 
					{
						...
						if (Selection().HasItem(dwTargetID))
						{
							pSubMenu->EnableMenuItem(ID_TDD_SETTASKDEPENDENCY, nDisabled);
							pSubMenu->EnableMenuItem(ID_TDD_ADDTASKDEPENDENCY, nDisabled);
						}
			...
			else if (bMove)
			{
				...
				m_data.MoveTasks(cache.aSelTaskIDs, dwDestParentID, dwDestPrevSiblingID);
	...
	default:
		{
			...
			BOOL bWantTrueTask = (m_crReference == CLR_NONE);
...
BOOL CToDoCtrl::DrawItemColumn(const TODOITEM* pTDI, const TODOSTRUCTURE* pTDS, const NCGDRAWITEM* pNCGDI, TDI_STATE nState, DWORD dwRefID)
{
	...
	case TDCC_PRIORITY:
		...
		if (!HasStyle(TDCS_DONEHAVELOWESTPRIORITY) || !m_data.IsTaskDone(pTDI, pTDS, TDCCHECKALL))
		{
			...
			if (m_data.IsTaskDue(pTDI, pTDS))
			{
				...
				if ((!bDueToday && m_crDue != CLR_NONE) || 
					(bDueToday && m_crDueToday != CLR_NONE))
				{
					POINT pt[3] = 
					{ 
	...
	case TDCC_PERCENT:
		{
			...
			CString sPercent = Misc::Format(nPercent, _T("%"));
			...
			if (HasStyle(TDCS_SHOWPERCENTASPROGRESSBAR))
			{
				...
				if (crBar != CLR_NONE)
				{
					...
					GraphicsMisc::DrawRect(pNCGDI->pDC, rProgress, CLR_NONE, crBorder);
	...
	case TDCC_TIMESPENT:
		{
			...
			int nUnits = s_tdDefault.GetTimeUnits(bTimeEst); // good default value
	...
	case TDCC_FLAG:
		if (pTDI->bFlagged)
			DrawGutterItemText(pNCGDI->pDC, FLAGCHAR, rItem, pNCGDI->nTextAlign, CLR_NONE, &GraphicsMisc::WingDings(), TRUE);
		break;
	...
	case TDCC_REMINDER:
		if (GetParent()->SendMessage(WM_TDCM_TASKHASREMINDER, dwTaskID, (LPARAM)this))
			DrawGutterItemText(pNCGDI->pDC, BELLCHAR, rItem, pNCGDI->nTextAlign, CLR_NONE, &GraphicsMisc::WingDings(), TRUE);
		break;
	...
	case TDCC_REMAINING:
		{
			...
			{
				sTime = CTimeHelper().FormatTime(dRemaining, nUnits, 2);
			}
			else // TDCS_CALCREMAININGTIMEBYDUEDATE
			{
				...
				{
					if (HasStyle(TDCS_DISPLAYHMSTIMEFORMAT))
						sTime = CTimeHelper().FormatTimeHMS(dRemaining, THU_DAYS, TRUE);
					else
					{
	...
	default:
		...
		if (!DrawItemCustomColumn(pTDI, pTDS, pNCGDI))
		{
			ASSERT(0);
			return FALSE;
		}
...
BOOL CToDoCtrl::DrawItemCustomColumn(const TODOITEM* pTDI, const TODOSTRUCTURE* pTDS, const NCGDRAWITEM* pNCGDI)
{
	TDC_COLUMN nColID = (TDC_COLUMN)pNCGDI->nColID;
	...
	case TDCCA_DATE:
		{
			...
			DrawGutterItemDate(pNCGDI->pDC, dDate, TDCD_CUSTOM, rItem);
		}
		break;
	...
	case TDCCA_DOUBLE:
		{
			...
			DrawGutterItemText(pNCGDI->pDC, Misc::Format(dValue, 2), rItem, pNCGDI->nTextAlign);
		}
		break;

	case TDCCA_INTEGER:
		{
			double dValue = data.AsInteger();
			m_data.CalcTaskCustomAttributeData(pTDI, pTDS, attribDef, dValue);

			DrawGutterItemText(pNCGDI->pDC, Misc::Format(dValue, 0), rItem, pNCGDI->nTextAlign);
		}
		break;
...
{
	// check header first
	if (m_tree.PtInHeader(ptScreen))
		return TDCHT_COLUMNHEADER;

	// then tree window rect
	CRect rTree;
	m_tree.GetWindowRect(rTree);

	if (!rTree.PtInRect(ptScreen))
	...
	// adjust to be in client area
	ptTree.x = max(ptTree.x, 0);

	if (!rTree.PtInRect(ptTree))
		return TDCHT_NOWHERE;

	// then hit task
	HTREEITEM htiHit = m_tree.HitTest(ptTree);

	return (htiHit ? TDCHT_TASK : TDCHT_TASKLIST);
}
...
BOOL CToDoCtrl::HandleCustomColumnClick(TDC_COLUMN nColID)
{
	...
	case TDCCA_ICON:
		if (attribDef.GetListType() == TDCCA_FIXEDLIST)
		{
			...
			BOOL bNext = (!Misc::KeyIsPressed(VK_SHIFT));
...
LRESULT CToDoCtrl::OnGutterRecalcColWidth(WPARAM /*wParam*/, LPARAM lParam)
{
	...
	case TDCC_ALLOCTO:
		{
			// determine the longest visible string
			CString sEdit = CDialogHelper::GetControlText(&m_cbAllocTo);
			CString sLongest = m_find.GetLongestAllocTo(sEdit);
	...
	case TDCC_CATEGORY:
		{
			// determine the longest visible string
			CString sEdit = CDialogHelper::GetControlText(&m_cbCategory);
			CString sLongest = m_find.GetLongestCategory(sEdit);
	...
	case TDCC_TAGS:
		{
			// determine the longest visible string
			CString sEdit = CDialogHelper::GetControlText(&m_cbTags);
			CString sLongest = m_find.GetLongestTag(sEdit);
	...
	case TDCC_LASTMOD:
		pNCRC->nWidth = CalcMaxDateColWidth(TDCD_LASTMOD, pNCRC->pDC, TRUE);
		break;

	case TDCC_DUEDATE:
		pNCRC->nWidth = CalcMaxDateColWidth(TDCD_DUE, pNCRC->pDC, !HasStyle(TDCS_HIDEDUETIMEFIELD));
		break;

	case TDCC_CREATIONDATE:
		pNCRC->nWidth = CalcMaxDateColWidth(TDCD_CREATE, pNCRC->pDC, TRUE);
		break;

	case TDCC_STARTDATE:
		pNCRC->nWidth = CalcMaxDateColWidth(TDCD_START, pNCRC->pDC, !HasStyle(TDCS_HIDESTARTTIMEFIELD));
		break;

	case TDCC_DONEDATE:
		pNCRC->nWidth = CalcMaxDateColWidth(TDCD_DONE, pNCRC->pDC, !HasStyle(TDCS_HIDEDONETIMEFIELD));
		break;
	...
	default:
		...
		if (CTDCCustomAttributeHelper::IsCustomColumn(nColID))
		{
			...
			else 
			{
				...
				{
				case TDCCA_DATE:
					pNCRC->nWidth = CalcMaxDateColWidth(TDCD_CUSTOM, pNCRC->pDC, FALSE);
					break;
...
int CToDoCtrl::CalcMaxDateColWidth(TDC_DATE nDate, CDC* pDC, BOOL bWantTime)
{
	// basic date string
	...
	// time component
	if (bWantTime && m_find.FindVisibleTaskWithTime(nDate))
...
void CToDoCtrl::AppendTaskFileHeader(CTaskFile& tasks) const
{
	...
	tasks.SetLastModified(m_tLastTaskMod);
...
int CToDoCtrl::GetSelectedTasks(CTaskFile& tasks, const TDCGETTASKS& filter, DWORD dwFlags) const
{
	...
	pos = Selection().GetFirstItemPos();

	while (pos)
	{
		DWORD dwSelID = Selection().GetNextItemData(pos);
		ASSERT(dwSelID);
								  ...
								  HTASKITEM hTask, const TDCGETTASKS& filter, BOOL bTitleCommentsOnly) const
{
	...
	ASSERT (!bTransform || bHtmlComments);
	...
	// actual text color
	COLORREF crText = CLR_NONE, crBack = CLR_NONE;
	GetTaskTextColors(pTDI, pTDS, crText, crBack);

	file.SetTaskTextColors(hTask, crText, crBack);
...
BOOL CToDoCtrl::EnsureSelectionVisible()
{
	if (GetSelectedCount())
	{
		CHoldRedraw hr(m_tree);
...
void CToDoCtrl::SetFocusToTasks()
{
	...
	if (pFocus != &m_tree)
	{
		// NOTE: if the comments were the last window focused
	...
	if (pTarget == &m_tree)
	{
		...
		if (pData->hti && (bSingleFile || bSingleOObj))
		{
			...
			if (bSingleOObj)
			{
				...
				sFile = COutlookHelper::FormatItemAsUrl(item);
			}
...
BOOL CToDoCtrl::CheckIn()
{
	...
	if (m_sLastSavePath.IsEmpty() || !m_bCheckedOut)
		return TRUE;
	...
	// before we try to restore the backup
	{
		...
		if (file.Open(m_sLastSavePath, XF_READWRITE, FALSE))
		{
			if (file.LoadEx())
			{
				...
				if (m_bSourceControlled)
				{
					...
					if (!MatchesSourceControlID(sCheckedOutTo))
					{
						// something has gone badly wrong because we think
						// we have it checked out but we don't really
						m_bCheckedOut = FALSE;
					}
					else
					{
						file.SetCheckedOutTo(""); // clear but retain the checkout item
...
BOOL CToDoCtrl::GotoFile(const CString& sUrl, BOOL bShellExecute)
{
	...
	else if (bShellExecute)
	{
		...
		if (CFileEdit::GotoFile(sUrl, sTasklistFolder, FALSE) >= 32)
			return TRUE;
	}

	// all else, pass to parent
	return GetParent()->SendMessage(WM_TDCM_FAILEDLINK, 0, (LPARAM)(LPCTSTR)sUrl);
}
...
BOOL CToDoCtrl::TaskHasIncompleteDependencies(DWORD dwTaskID, CString& sIncomplete) const
{
	...
	for (int nDepends = 0; nDepends < nNumDepends; nDepends++)
	{
		...
		{
			BOOL bDependentIsDone = GetParent()->SendMessage(WM_TDCM_TASKISDONE, dwDependID, (LPARAM)(LPCTSTR)sFile);
...
int CToDoCtrl::SortTasks(LPARAM lParam1, LPARAM lParam2, TDC_COLUMN nSortBy, BOOL bAscending, 
						 BOOL bSortDueTodayHigh, DWORD dwTimeTrackedID, const CToDoCtrl* pTDC,
						 const CTDCCustomAttribDefinitionArray* pAttibuteDefs)
{
	...
	if (nSortBy == TDCC_TRACKTIME)
	{
		...
		return (bAscending ? (bTracked1 - bTracked2) : (bTracked2 - bTracked1));
	}
	else if (nSortBy == TDCC_REMINDER)
	{
		BOOL bHasReminder1 = (pTDC->GetParent()->SendMessage(WM_TDCM_TASKHASREMINDER, dwTaskID1, (LPARAM)pTDC) ? 1 : 0);
		BOOL bHasReminder2 = (pTDC->GetParent()->SendMessage(WM_TDCM_TASKHASREMINDER, dwTaskID2, (LPARAM)pTDC) ? 1 : 0);

		return (bAscending ? (bHasReminder1 - bHasReminder2) : (bHasReminder2 - bHasReminder1));
	}
	// handle custom attribute
	else if (nSortBy >= TDCC_CUSTOMCOLUMN_FIRST && nSortBy <= TDCC_CUSTOMCOLUMN_LAST)
	{
		TDCCUSTOMATTRIBUTEDEFINITION attribDef;
		VERIFY(CTDCCustomAttributeHelper::GetAttributeDef(nSortBy, *pAttibuteDefs, attribDef));

		return pTDC->m_data.CompareTasks(dwTaskID1, dwTaskID2, attribDef, bAscending);
	}

	// else default attribute
	return pTDC->m_data.CompareTasks(dwTaskID1, dwTaskID2, nSortBy, bAscending, bSortDueTodayHigh);
}
