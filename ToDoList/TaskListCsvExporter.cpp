...
bool CTaskListCsvExporter::InitConsts(const ITaskList10* pTasks, LPCTSTR szDestFilePath, BOOL bSilent, IPreferences* pPrefs, LPCTSTR szKey)
{
	...
	// Note: Do parent ID first so that task ID is inserted at start
	CheckAddIDField(TRUE);
	CheckAddIDField(FALSE);

	return true;
}

void CTaskListCsvExporter::CheckAddIDField(BOOL bParentID)
{
	TDC_ATTRIBUTE nAttrib = (bParentID ? TDCA_PARENTID : TDCA_ID);
