...
CUrlRichEditCtrl::CUrlRichEditCtrl() : m_nContextUrl(-1), m_nFileProtocol(-1), m_lpDragObject(NULL)
{
										 ...
										 DWORD /*reco*/, BOOL fReally, HGLOBAL /*hMetaPict*/)
{
	...
	if (bEnable)
	{
		...
		if (fReally && (*lpcfFormat == COutlookHelper::CF_OUTLOOK))
		{
			...
			CString sLinks(m_crDropSel.cpMin == 0 ? _T("") : _T("\n"));

			// 1-based indexing
			for (short nSel = 1; nSel <= pSelection->GetCount(); nSel++)
			{
				OutlookAPI::_MailItem item(pSelection->Item(COleVariant(nSel)));

				sLinks += COutlookHelper::FormatItemAsUrl(item) + _T("\n");
			}
			...
			if (!sLinks.IsEmpty())
			{
				...
				SetSel(m_crDropSel.cpMin, m_crDropSel.cpMin + sLinks.GetLength());
				SetFocus();

				// check that the Outlook Url handler is installed
				//COutlookHelper::QueryInstallUrlHandler();
...
{
	CString sClipText = Misc::GetClipboardText(*this);

	// try pasting a file path
	if (FileMisc::IsPath(sClipText))
	{
