...
int CMenuIconMgr::AddImages(const CUIntArray& aCmdIDs, UINT nIDBitmap, int nCx, COLORREF crMask)
{
	...
	if (bmDis.LoadBitmap(nIDBitmap))
		GraphicsMisc::GrayScale(bmDis, crMask);

	return AddImages(aCmdIDs, bm, bmDis, nCx, crMask);
}

int CMenuIconMgr::AddImages(const CUIntArray& aCmdIDs, const CString& sImagePath, int nCx, COLORREF crMask)
{
	CEnBitmap bm, bmDis;
...
int CMenuIconMgr::AddImages(const CUIntArray& aCmdIDs, CBitmap& bm, CBitmap& bmDis, int nCx, COLORREF crMask)
{
	if (bm.GetSafeHandle())
	{
		CImageList il, ilDis;
