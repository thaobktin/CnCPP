...
CScrollingPropertyPageHost::CScrollingPropertyPageHost() : CPropertyPageHost(), m_crBack(CLR_NONE)
{
...
void CScrollingPropertyPageHost::SetBackgroundColor(COLORREF color)
{
	m_crBack = color;
...
void CScrollingPropertyPageHost::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	if (m_scroll.IsWindowVisible())
	{
		...
		if (si.nPos != nOrgPos)
		{
			...
			rPage.OffsetRect(0, (nOrgPos - si.nPos));
