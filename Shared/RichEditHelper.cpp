...
{
	InitRichEdit();
	return wnd.Create(WC_RICHEDIT50, _T(""), dwStyle, rect, pParentWnd, nID);
}
...
BOOL CRichEditHelper::PasteFiles(CWnd& wnd, const CStringArray& aFiles, RE_PASTE nPasteHow)
{
	...
	CClipboardBackup clipBackup(wnd);
	clipBackup.Backup();
	...
	for (UINT i=0; i < nFileCount; i++)
	{
		...
		if (PasteFileInternal(wnd, sFile, nPasteHow))
			nFilesPasted++;
	}
	...
	if (FileMisc::PathExists(szFilePath)) // can be file or folder
	{
		...
		case REP_ASICONLINK:
			...
			if (!FileMisc::FolderExists(szFilePath) && 
				...
				CReFileObject(wnd).Insert(szFilePath, (nPasteHow == REP_ASICONLINK)))
			{
				return TRUE;
			}
			// else fall thru
