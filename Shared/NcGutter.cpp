...
/*
void CNcGutter::InitScrollInfo(SCROLLINFO& si)
{
...
void CNcGutter::ClearScrollInfo()
{
	InitScrollInfo(m_lastVScrollInfo);
}

BOOL CNcGutter::IsScrollInfoSet() const
{
	SCROLLINFO siNull;
	InitScrollInfo(siNull);

	return (memcmp(&m_lastVScrollInfo, &siNull, siNull.cbSize) != 0);
}

void CNcGutter::UpdateScrollInfo()
{
	GetScrollInfo(m_lastVScrollInfo);
}

void CNcGutter::GetScrollInfo(SCROLLINFO& siVert) const
{
	InitScrollInfo(siVert);
	::GetScrollInfo(GetHwnd(), SB_VERT, &siVert);
}

int CNcGutter::CalcVertScrollOffset(SCROLLINFO& siFrom, SCROLLINFO& siTo)
{
	int nItemHeight = GetItemHeight();

	int nOffset = (siTo.nPos - siFrom.nPos) * nItemHeight;

	return nOffset;
}

BOOL CNcGutter::HasScrollInfoChanged() const
{
	SCROLLINFO siVert;
	GetScrollInfo(siVert);

	return (memcmp(&m_lastVScrollInfo, &siVert, siVert.cbSize) != 0);
}
*/
...
{
	//static int n = 0;
	//TRACE (_T("CNcGutter::RecalcGutter(%d)\n"), ++n);
...
CFont* CNcGutter::PrepareFont(CDC* pDC, BOOL bHeader, HFONT hFont)
{
	...
	if (IsHooked())
		pWnd = (bHeader ? GetCWnd()->GetParent() : GetCWnd());

	return GraphicsMisc::PrepareDCFont(pDC, pWnd, pFont);
}
...
void CNcGutter::OnNcPaint() 
{
	...
	if (nGutterWidth > 0)
	{
		...
		// reuse as much of the previous bitmap as we can
		// if the scroll info is not set then that means we're here
		// for the first time so we just proceed without trying
		// to reuse the bitmap
// 		BOOL bScrollInfoSet = IsScrollInfoSet();
// 		BOOL bScrollInfoChanged = HasScrollInfoChanged();

		// see if there's anything to draw
//		BOOL bChange = (bNewBitmap || !bScrollInfoSet || bScrollInfoChanged);
			
//		if (bChange)
		{
/*			if (bScrollInfoSet && bScrollInfoChanged)
			{
				SCROLLINFO siVert;
				GetScrollInfo(siVert);

				// compare to existing
				int nVOffset = CalcVertScrollOffset(m_lastVScrollInfo, siVert);

				if (nVOffset)
				{
					// clip out the header rect
					CRect rClip(rGutter);

					if (HasStyle(NCGS_SHOWHEADER))
						rClip.top = rHeader.bottom;

					TRACE (_T("CNcGutter::OnNcPaint(Mem DC scrolled by %d)\n"), -nVOffset);
					m_dcMem.ScrollDC(0, -nVOffset, rClip, rClip, NULL, NULL);

					if (nVOffset > 0)
						rGutter.top += (siVert.nPage * GetItemHeight() - nVOffset);
					else
						rGutter.bottom = rGutter.top - nVOffset;
				}
			}
...
void CNcGutter::NcDrawHeaderColumn(CDC* pDC, int nColumn, CRect rColumn, CThemed* pTheme, LPPOINT pCursor)
{
	...
	if (!pCD->sTitle.IsEmpty())
	{
		...
		if (pCD->hFont)
		{
			...
			if (pCD->bSymbolFont)
				GraphicsMisc::DrawSymbol(pDC, (char)pCD->sTitle[0], rText, DEFFLAGS | pCD->nTextAlign, CFont::FromHandle(pCD->hFont));
			else
