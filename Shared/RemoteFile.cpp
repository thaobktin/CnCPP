...
/*				char szFilename[_MAX_FNAME], szExt[_MAX_EXT];
				_tsplitpath(aLocalFiles[nFile], NULL, NULL, szFilename, szExt);
				
				char szRemotePath[MAX_PATH+1];
				_tmakepath(szRemotePath, NULL, sRemotePath, szFilename, szExt);
*/				
				aRemoteFiles.Add(FILERESULT(sRemoteFile));
			}
		}
...
CString CRemoteFile::GetTempPath(const CString& sRemotePath, DWORD dwOptions)
{
  ...
  if (bKeepFName)
    sTempPath = FileMisc::GetTempFileName(FileMisc::GetFileNameFromPath(sRemotePath), _T(""));
  else
  {
    sTempPath = FileMisc::GetTempFileName(_T("rmf"));
...
BOOL CRemoteFile::SetProxy(const CString& sProxy, UINT nPort)
{
	...
	if (!sProxy.IsEmpty())
		sProxyAndPort.Format(_T("ftp=ftp://%s:%d"), sProxy, nPort);
