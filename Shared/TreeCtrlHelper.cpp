...
int CTreeCtrlHelper::BuildHTIMap(CHTIMap& mapHTI) const
{
	mapHTI.RemoveAll();
...
HTREEITEM CTreeCtrlHelper::FindDeepestTask(BOOL bExpandedOnly) const
{
	int nDepth = 0;
	return FindDeepestTask(NULL, nDepth, bExpandedOnly);
}

HTREEITEM CTreeCtrlHelper::FindDeepestTask(int& nDepth, BOOL bExpandedOnly) const
{
	nDepth = 0;
	return FindDeepestTask(NULL, nDepth, bExpandedOnly);
}

HTREEITEM CTreeCtrlHelper::FindDeepestTask(HTREEITEM htiParent, int& nDepth, BOOL bExpandedOnly) const
{
	...
	while (htiChild)
	{
		...
		HTREEITEM htiChildDeepest = FindDeepestTask(htiChild, nChildDepth, bExpandedOnly);
