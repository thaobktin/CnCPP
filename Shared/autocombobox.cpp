...
void CAutoComboBox::HandleReturnKey()
{
	...
	if (IsHooked())
	{
		...
		if (nAdd != CB_ERR)
		{
			ParentNotifyItemAdded(nAdd, sEdit);
		}
...
void CAutoComboBox::ParentNotifyItemAdded(int nIndex, LPCTSTR szItem)
{
	...
	if ((nIndex >= 0) && (!szItem || szItem[0]))
	{
		COwnerdrawComboBoxBase::GetParent()->SendMessage(WM_ACB_ITEMADDED, 
												MAKEWPARAM(CWnd::GetDlgCtrlID(), nIndex),
												(LPARAM)szItem);
	}
}

void CAutoComboBox::ParentNotify(UINT nIDNotify)
{
