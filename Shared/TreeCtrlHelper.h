...
public:
	...
	HTREEITEM FindDeepestTask(int& nDepth, BOOL bExpandedOnly = FALSE) const;
	HTREEITEM FindDeepestTask(BOOL bExpandedOnly = FALSE) const;

	void EnsureVisibleEx(HTREEITEM hti, BOOL bVPartialOK = TRUE, BOOL bHPartialOK = TRUE);

	int BuildHTIMap(CHTIMap& mapHTI) const;
...
protected:
	...
	HTREEITEM FindDeepestTask(HTREEITEM htiParent, int& nDepth, BOOL bExpandedOnly) const;
