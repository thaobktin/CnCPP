...
public:
	...
	LPCTSTR GetTypeID() const { return STATISTICS_TYPEID; }
...
protected:
	void UpdateTask(const ITaskList12* pTasks, HTASKITEM hTask, IUI_UPDATETYPE nUpdate, int nEditAttribute);
	void UpdateDataExtents();
	BOOL RemoveDeletedTasks(const ITaskList12* pTasks);
	void BuildData(const ITaskList12* pTasks);
	void BuildData(const ITaskList12* pTasks, HTASKITEM hTask);
	void SortData();
