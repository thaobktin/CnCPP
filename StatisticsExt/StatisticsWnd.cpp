...
void CStatisticsWnd::BuildData(const ITaskList12* pTasks)
{
	...
	if (pTasks->GetTaskCount())
	{
		...
		BuildData(pTasks, pTasks->GetFirstTask());
...
void CStatisticsWnd::BuildData(const ITaskList12* pTasks, HTASKITEM hTask)
{
	if (hTask == NULL)
		return;
...
void CStatisticsWnd::UpdateTask(const ITaskList12* pTasks, HTASKITEM hTask, IUI_UPDATETYPE nUpdate, int nEditAttribute)
{
	...
	// children
	UpdateTask(pTasks, hChild, nUpdate, nEditAttribute);
}
...
void CStatisticsWnd::DoAppCommand(IUI_APPCOMMAND nCmd) 
{ 
	switch (nCmd)
	{
	case IUI_EXPANDALL:
...
bool CStatisticsWnd::CanDoAppCommand(IUI_APPCOMMAND nCmd) const 
{ 
	switch (nCmd)
	{
	case IUI_EXPANDALL:
...
void CStatisticsWnd::OnSize(UINT nType, int cx, int cy) 
{
	...
	{
		CDialogHelper::ResizeCtrl(this, IDC_DIVIDER, cx, 0);
...
COleDateTime CStatisticsWnd::GetGraphStartDate() const
{
	...
	case SCALE_2MONTH:
		st.wDay = 1; // start of month;
		st.wMonth -= ((st.wMonth - 1) % 2); // previous even month
		break;

	case SCALE_QUARTER:
		st.wDay = 1; // start of month;
		st.wMonth -= ((st.wMonth - 1) % 3); // previous quarter
		break;
		
	case SCALE_HALFYEAR:
		st.wDay = 1; // start of month;
		st.wMonth -= ((st.wMonth - 1) % 6); // previous half-year
		break;
...
COleDateTime CStatisticsWnd::GetGraphEndDate() const
{
	...
	case SCALE_MONTH:
		st.wDay = CDateHelper::GetDaysInMonth(st.wMonth, st.wYear); // end of month;
		break;
