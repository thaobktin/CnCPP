...
{
	if ((m_crText == -1 && m_crFrom == -1 && m_crTo == -1) || !CThemed::IsThemeActive())
		Default();
	else
	{
		CPaintDC dc(this);
...
void CStatusBarACT::DrawRectBkgnd(CDC* pDC, const CRect& rect)
{
	...
	else
	{
		if (m_bGradient && m_bGlass)
			GraphicsMisc::DrawGlassWithGradient(pDC, rect, m_crFrom, m_crTo, FALSE);

		else if (m_bGradient)
			GraphicsMisc::DrawGradient(pDC, rect, m_crFrom, m_crTo, FALSE);
		else
			GraphicsMisc::DrawGlass(pDC, rect, m_crFrom, m_crTo, FALSE);
	}
}
