...
CString Base64Coder::Encode(LPCTSTR szMessage)
{
	if (szMessage != NULL)
	{
		Base64Coder::Encode((const PBYTE)szMessage, lstrlen(szMessage) * sizeof(TCHAR));
...
CString Base64Coder::Decode(LPCTSTR szMessage)
{
	if (szMessage != NULL)
	{
		Base64Coder::Decode((const PBYTE)szMessage, lstrlen(szMessage) * sizeof(TCHAR));
