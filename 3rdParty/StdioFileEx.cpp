...
BOOL CStdioFileEx::Open(LPCTSTR lpszFileName, UINT nOpenFlags, SFE_SAVEAS nSaveAs, CFileException* pError /*=NULL*/)
{
	...
	if (bOK && GetLength() >= max(UTF8_BOM_SIZE, UNICODE_BOM_SIZE))
	{
		// check for unicode BOM
		wchar_t	cBOMTest;
		Read(&cBOMTest, UNICODE_BOM_SIZE);
		...
		else // not unicode
		{
			...
			// check for UTF8 BOM
			if (CP_UTF8 == m_nFileCodePage)
			{
				BYTE	arrUTF8BOMTest[UTF8_BOM_SIZE];
				Read(arrUTF8BOMTest, UTF8_BOM_SIZE);
				
				m_bWriteBOM = (memcmp(&arrUTF8BOMTest, UTF8_BOM, UTF8_BOM_SIZE) == 0);
			}
		}
