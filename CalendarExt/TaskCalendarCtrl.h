...
{
	TASKCALITEM();
	TASKCALITEM(const ITaskList12* pTasks, HTASKITEM hTask, DWORD dwCalcDates);

	void UpdateTask(const ITaskList12* pTasks, HTASKITEM hTask, int nEditAttrib, DWORD dwCalcDates);
...
protected:
	void UpdateTaskDates(const ITaskList12* pTasks, HTASKITEM hTask, int nEditAttrib, DWORD dwCalcDates);
...
public:
	...
	BOOL GetTaskLabelRect(DWORD dwTaskID, CRect& rLabel);
...
// Overrides
	...
	void BuildData(const ITaskList12* pTasks, HTASKITEM hTask, BOOL bAndSiblings = TRUE);
	void DeleteData();
	...
	int GetTaskVertPos(DWORD dwTaskID) const;
...
//	BOOL WantShowTaskAsContinuous(const TASKCALITEM* pTCI) const;
	...
	void RemoveCompletedTask(const ITaskList12* pTasks, HTASKITEM hTask);
