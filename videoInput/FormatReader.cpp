...
FormatReader& FormatReader::getInstance()
{
	static FormatReader instance;

	return instance;
}

unsigned int *FormatReader::GetParametr(GUID guid, MediaType &out)
{
...
HRESULT FormatReader::LogAttributeValueByIndexNew(IMFAttributes *pAttr, DWORD index, MediaType &out)
{
    ...
    if (hr == S_FALSE)
    {
        ...
        case VT_CLSID:
			...
			{
				hr = GetGUIDNameNew(*var.puuid, guidValName);

				if (SUCCEEDED(hr))
				{
					out.MF_MT_AM_FORMAT_TYPE = MF_MT_AM_FORMAT_TYPE;

					out.MF_MT_AM_FORMAT_TYPEName = guidValName;
				}
			}
			...
			if(guid == MF_MT_MAJOR_TYPE)
			{
				...
				if (SUCCEEDED(hr))
				{
					...
					out.MF_MT_MAJOR_TYPEName = guidValName;
				}
			}
			...
			if(guid == MF_MT_SUBTYPE)
			{
				...
				if (SUCCEEDED(hr))
				{
					...
					out.MF_MT_SUBTYPEName = guidValName;
				}
			}
...
HRESULT FormatReader::GetGUIDNameNew(const GUID& guid, std::wstring &nameGUID)
{
    HRESULT hr = S_OK;
    ...
    {
       nameGUID = std::wstring(pcwsz);
    }

    return hr;
}

void FormatReader::LogUINT32AsUINT64New(const PROPVARIANT& var, UINT32 &uHigh, UINT32 &uLow)
{
    Unpack2UINT32AsUINT64(var.uhVal.QuadPart, &uHigh, &uLow);

}

HRESULT FormatReader::SpecialCaseAttributeValueNew(GUID guid, const PROPVARIANT& var, MediaType &out)
{
	...
	if (guid == MF_MT_FRAME_RATE)
	{
		...
		out.MF_MT_FRAME_RATE = (float)uHigh/(float)uLow;
	}
	else
	if (guid == MF_MT_FRAME_RATE_RANGE_MAX)
	{
		...
		out.MF_MT_FRAME_RATE_RANGE_MAX = (float)uHigh/(float)uLow;
	}
	else
	if (guid == MF_MT_FRAME_RATE_RANGE_MIN)
	{
		...
		out.MF_MT_FRAME_RATE_RANGE_MIN = (float)uHigh/(float)uLow;
	}
	else
	if (guid == MF_MT_PIXEL_ASPECT_RATIO)
	{
		...
		out.MF_MT_PIXEL_ASPECT_RATIO = (float)uHigh/(float)uLow;
	}	     
    else
    {
        return S_FALSE;
...
LPCWSTR FormatReader::GetGUIDNameConstNew(const GUID& guid)
{
