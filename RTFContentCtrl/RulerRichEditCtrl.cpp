...
const char* DEFAULTRTF = "{\\rtf1\\ansi\\deff0 \\f0\\fs60   }";
...
struct STREAMINCOOKIE
{
	...
	int GetLength() const { return sRTF.GetLength() * sizeof(TCHAR); }
	LPCSTR CopyFrom() const { return ((LPCSTR)(LPCTSTR)sRTF) + nStreamPos; }
...
BOOL CRulerRichEditCtrl::Save(CString& filename)
{
	...
	FileMisc::SaveFile(filename, *str);
...
void CRulerRichEditCtrl::DoIndent()
{
	...
	ParaFormat	para(PFM_STARTINDENT | PFM_TABSTOPS/* | PFM_NUMBERING*/);
	/*DWORD dwFormat = */m_rtf.GetParaFormat(para);
	...
	para.Reset(PFM_OFFSET | PFM_OFFSETINDENT | PFM_TABSTOPS);
	para.dxStartIndent = (newindent - curindent);

	// if we are indenting a numbered list then restart
	// the numbering ie. treat as a subtask
// 	if (dwFormat & PFM_NUMBERING)
// 	{
// 		para.dwMask |= (PFM_NUMBERING | PFM_NUMBERINGSTART | PFM_NUMBERINGSTYLE);
// 		para.wNumberingStart = 1;
// 		para.wNumberingStyle = 0x300;
// 	}
